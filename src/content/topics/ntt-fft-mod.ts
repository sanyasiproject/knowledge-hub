 ...
 */
export const nttFftMod: TopicContent = {
  quickSummary: [
    "Computes convolutions modulo a prime without overflow/precision loss, using coefficient splitting (or NTT-friendly primes).",
    "Needed when answers must be exact under mods like 1e9+7 where plain doubles lose precision.",
  ],
  code: [
    {
      language: "cpp",
      caption: "Template \u2014 FFT under a Modulus",
...
vector<int> mult_mod(const vector<int> &a, const vector<int> &b)
{
  ...
  cheatSheet: [
    "Plain FFT is unsafe past ~1e14 magnitude \u2014 use this for mod answers.",
    "Split coefficients (a = hi\u00b72\u00b9\u2075 + lo) and combine three transforms.",
  ],
};
