...
export const linearDiophantine: TopicContent = {
  quickSummary: [
    "Extended Euclid gives one solution of ax + by = g; scale to c when g | c, else no solution exists.",
    "All solutions form an arithmetic family: x + k\u00b7(b/g), y - k\u00b7(a/g).",
  ],
  code: [
    {
      language: "cpp",
      caption: "Template \u2014 Linear Diophantine Equations",
      source: `int extended_euclid(int a, int b, int &x, int &y) {
    int xx = y = 0;
    int yy = x = 1;
    while (b) {
        int q = a / b;
...
int find_all_solutions(int a, int b, int c, int &minx, int &maxx, int &miny, int &maxy)
{
  ...
  cheatSheet: [
    ...
    "Shift k to satisfy range constraints on x or y.",
  ],
};
